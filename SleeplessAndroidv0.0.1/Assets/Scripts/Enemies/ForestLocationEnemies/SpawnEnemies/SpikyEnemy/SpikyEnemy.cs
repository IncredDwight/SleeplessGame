...
public class SpikyEnemy : Enemy
{
    ...
    protected void Awake()
    {
        EnemySetUp(3, 30, 20);
        _coolDown = 1.5f;
        if (PoolManager.Instance.GetPool("Spike(Clone)") != null)
            _projectilesPool = PoolManager.Instance.GetPool("Spiky(Clone)");
        else
        {
            _projectile = FindObjectOfType<Spike>().gameObject;
            _projectilesPool = PoolManager.Instance.AddPool(_projectile, _projectile.name, 20);
    ...
    private void SpawnSpikies(int spikesAmount)
    {
        ...
        for (int i = 0; i <= spikesAmount; i++)
        {
            ...
            Quaternion rot = Quaternion.FromToRotation(transform.up, center - pos);
